...
if len(sys.argv) < 3:
    print("Usage: python parse_rtt_tcp.py directory evaluatedprefix")
    exit(1)
...
filename = data_dir + '/eval.raw'
out_thr = data_dir + '/rtt.dat'
        ...
        if src.startswith(prefix):
            ...
            for suitor in unmatched_sent[key][:]:
                if suitor.get("expected_ack") == int(packet.get("ack_num")):
                    ...
                    out_latency_file.write(packet.get("src") + " " + packet.get("dst")+ " " + packet.get("ack_num") + " "+ str(time_diff) + '\n')
